...
The reference document for **linea**, a minimalist, writing-focused note-taking PWA backed by Google Sheets. It describes the app as it actually is today, so that any future session or developer can work on it accurately. Read it before changing code.

**Note on history:** the original version of this document described a much simpler, single-device, six-column app with no dark mode and a monospace editor. The app has since grown significantly. This document supersedes that entirely.
...
- Session key `linea_gtoken`: cached OAuth token (sessionStorage), including `_expiresAt` (expiry timestamp with 60s safety margin).
- Per-note local-only flag `dirty`: created/edited offline, awaiting sync. Not written to the Sheet.
- `deletedIds` is a **tombstone list** — IDs of permanently deleted notes, to stop deleted notes being resurrected by a sync from another device. Capped at 1,000 entries.
...
- localStorage writes while typing are debounced separately at 1000 ms (`scheduleStorageSave`). A `visibilitychange` listener flushes any pending write immediately when the tab is hidden or closed.
...
3. `mergeNotes()` merges local + remote:
   ...
   - **The deletion pass is skipped when the remote is empty** — an empty sheet plus a populated local cache almost certainly means a misconfigured Sheet ID, not a genuine "user deleted everything" event.
...
5. Any `dirty` notes are pushed to the Sheet. **Empty dirty notes (abandoned drafts) are discarded** at this point rather than pushed — prevents "Untitled" ghost rows appearing in the Sheet.

### API operations

- Read: `GET Notes!A2:G`.
- Append (new note): `values.append` to `Notes!A:G`, `RAW`. Row number parsed from the returned `updatedRange`.
- Update: before writing, `verifyRow()` reads `Notes!A{row}` to confirm the row still holds the expected note ID. If it doesn't (rows shifted due to a deletion on another device), `sheetReadAll()` rebuilds the rowMap and retries; if the note still isn't found, it is appended as a new row.
- Delete: `batchUpdate` with `deleteDimension`. The Notes tab is found **by title** (`'Notes'`), not by index — guards against tab reordering. After deletion, every `rowMap` entry for rows below the deleted one is decremented by one.
...
### Import vs sync merge

- **`mergeNotes(local, remote)`** — used only by `syncFromSheet`. Can delete local notes absent from the remote (the post-merge pass). Never use for import.
- **`importMerge(local, imported)`** — used only by `importNotes`. Additive only: adds new notes, updates existing ones if the imported copy is newer, never deletes. Overrides tombstones so a restored backup can resurrect previously deleted notes. Marks all taken notes as `dirty` so they push to the Sheet on next sync.
...
**Loading screen** — shown until Google scripts resolve (or a 10s failsafe in `index.html` force-reveals the app). Background and logo/status colours are inline critical CSS and **must match the token values exactly**.
...
- **Chrome** (back, sync icon, copy, overflow menu) lives in the top bar and **fades after 2s of mouse stillness**, returns instantly on mouse movement (200ms ease). Stays hidden while typing. Revealed on keyboard focus (Tab) as well as mouse movement. On **touch devices it stays visible** (no hover model). Exit via the chrome or the **Escape** key.
...
- Ctrl/Cmd+Shift+D inserts a date stamp into the note (kept; no on-screen hint).
...
- Archive → leaves the list, toast with Undo (5s).
...
- Cache name is versioned (currently **`linea-v6`**). **Bump it on every meaningful deploy** or updates won't come through.
...
- localStorage write debounce **1000 ms** (`scheduleStorageSave`); flushed on `visibilitychange → hidden`.
- Pinned status stored in **Sheet column G** (cross-device).
- **Import uses `importMerge`** (additive, tombstone-overriding) — never `mergeNotes`. These two functions have different contracts; do not conflate them.
- **`sheetUpdate` verifies column A** before writing (`verifyRow`); rebuilds `rowMap` via `sheetReadAll` if the row has shifted; falls through to append if the note still can't be found.
- **`sheetDelete` finds the Notes tab by title**, not by index. After deletion it decrements all `rowMap` entries for rows below the deleted one.
- **The sheet-sync deletion pass is skipped when the remote is empty** (`allowDeletions: remote.length > 0`).
- Tombstones (`deletedIds`) **capped at 1,000** entries (newest kept).
...
- Editor is **chrome-light**: no metadata, fade-on-stillness (mouse + keyboard focus), three-state sync icon.
...
- Dark mode currently follows `prefers-color-scheme` only — an in-app manual toggle was discussed but not implemented.
